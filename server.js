...
app.post('/save-history', (req, res) => {
  ...
  const filePath = path.join(__dirname, 'src', 'history', 'historyCalculator.json');
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    if (existingEntry) {
      return res.status(400).json({ code: 400, message: 'History entry with this ID already exists.' });
    }
    ...
    fs.writeFile(filePath, JSON.stringify(historyArray, null, 2), (err) => {
      if (err) {
        return res.status(500).json({ code: 500, message: 'Error saving history' });
      }
      res.status(200).json({ code: 200, message: 'History saved successfully', history: historyEntry });
    });
  });
});
...
app.get('/load-history', (req, res) => {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    if (err) {
      return res.status(500).json({ code: 500, message: 'Error reading history' });
    }

    // Kiểm tra xem dữ liệu có rỗng hay không
    if (!data.trim()) {
      return res.status(200).json({ code: 200, message: 'History is empty', history: [] });
    }
    ...
    try {
      ...
      return res.status(500).json({ code: 500, message: 'Error parsing history data: ' + parseError.message });
    }
  });
});
